...
func CreateSeckillVoucher(ctx context.Context, c *app.RequestContext) {
	var err error
	var req voucher.SeckillVoucher
	err = c.BindAndValidate(&req)
	if err != nil {
		utils.SendErrResponse(ctx, c, consts.StatusOK, err)
		return
	...
	resp, err := service.NewCreateSeckillVoucherService(ctx, c).Run(&req)
